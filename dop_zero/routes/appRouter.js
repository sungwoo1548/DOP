...
router.post("/resister", (req, res, next) => {
  ...
  ibmdb.open(dsn2, function(err, conn) {
    conn.prepare(
      "insert into PERSONAL_INFO (person_name, person_birth, person_tel) VALUES (?, ?, ?)",
      function(err, stmt) {
        if (err) {
          //에러처리
          console.log(err);
        ...
        stmt.execute([person_name, person_birth, person_tel], function(
          err,
          result
        ) {
    ...
    conn.prepare(
      "insert into USER_INFO (person_name, user_id, user_pw) VALUES (?, ?, ?)",
      function(err, stmt2) {
        if (err) {
          //에러처리
          console.log(err);
...
router.post("/login", (req, res, next) => {
  ...
  ibmdb.open(dsn2, function(err, conn) {
    ibmdb.open(dsn2, function(err, connection) {
      ...
      connection.query(
        `select * from USER_INFO where user_id='${escape(req.body.userId)}'`,
        function(err1, readData) {
